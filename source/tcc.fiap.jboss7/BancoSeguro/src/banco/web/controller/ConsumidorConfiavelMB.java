 ...
 *   cadastro de aplicacoes confiaveis do Servico Web do Banco Seguro
 *   'a camada de visualizacao do JSF. 
 * @author Robson Martins (robson@robsonmartins.com)
 */
@SuppressWarnings("serial")
public class ConsumidorConfiavelMB extends AbstractBancoSeguroMB {
	
	/* objeto app selecionado na camada de visualizacao */
	private ConsumidorConfiavel appSelecionada;

	/* lista de consumidores confiaveis cadastrados no BancoSeguro */
	private List<ConsumidorConfiavel> apps;
	
	/**
	 * Cria uma nova instancia do controller.
	 * @param engine "Motor" do BancoSeguro (camada model).
	 */
	public ConsumidorConfiavelMB(IBancoSeguroEngine engine) {
		super(engine);
	...
	public void uploadCertAppConfiavel(FileUploadEvent event) {
		try {
			bancoEngine.adicionarAppConfiavel(event.getFile().getInputstream());
	...
	public void removerAppConfiavel() {
		if (JBossUtil.TCC_FIAP_RESTRICT_USER_CHANGES) {
			...
			try {
				bancoEngine.removerAppConfiavel(appSelecionada.getDname());
	...
	private void atualizarListaApps() {
		apps.clear();
		apps.addAll(bancoEngine.listarAppsConfiaveis());
	}
}
